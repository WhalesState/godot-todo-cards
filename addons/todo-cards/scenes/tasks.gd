...
func add_task(_text:= 'New Task', _pressed:= false)-> void:
	...
	_check_box.connect('pressed', self, '_on_checkbox_pressed', [_task])
...
func _on_checkbox_pressed(_task: HBoxContainer)-> void:
	var _col := [Color('ccffffff'), Color('98C379')]
	var _id := int(_task.get_node('CheckBox').pressed)
	var _line_edit = _task.get_node('LineEdit') as LineEdit
	_line_edit.add_color_override('font_color', _col[_id])
	set_progress()
#<END>
